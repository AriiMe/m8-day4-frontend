...
export default class NewStory extends Component {
  ...
  verifyUser = async () => {
    try {
      let response = await fetch(
        "http://localhost:6969/authors/?name=" + this.state.authorName
      );
  ...
  postArticle = async (authorID) => {
    try {
      ...
      if (this.props.match.params.slug) {
        await fetch(
          "http://localhost:6969/medium/" + this.props.match.params.slug,
          {
            method: "PUT",
        ...
        let response = await fetch("http://localhost:6969/medium/", {
          method: "POST",
          body: JSON.stringify(body),
          headers: {
            "Content-Type": "application/json",
